...
handle multiple minima better: if two models end in different basins,
weight averaging can land in a pathological midpoint, while output
averaging always gives a valid distribution.
...
\paragraph{Endpoint must be different enough.} We tested $T_A=100,
T_B=150$ (50\% more). We have not systematically explored other ratios
(e.g., $T_B=120$ vs.\ $T_B=200$). The ``sweet spot'' for the
trajectory-length gap remains an open empirical question.
...
\paragraph{Not a universal cure.} \ours{} is a small (+1--1.5 pp) gain
on top of standard deep ensembles. It is not a replacement for better
architectures, better data, or better pretraining. We view it as a
``free'' trick for practitioners who already use deep ensembles.
