...
Method & BACC \\
\midrule
CBraMod \citep{wang2025cbramod}    & $0.551$  \\
...
\bottomrule
\end{tabular}
\subcaption*{(a) Headline. Our best single model is $+7.4\%$ over EMOD;
the $10$-checkpoint ensemble $+10.5\%$. Ensembling the EMOD baseline at the
same $5$-seed budget reaches only $0.655$, so the ensemble-vs-ensemble
lead is $+0.039$ ($+6.0\%$).}
\end{minipage}\hfill
...
with random orthonormal directions is statistically equivalent
(Table~\ref{tab:sota}), so KD imposes a
...
$[-0.004,+0.010]$). The improvement is the recipe, not the ensembling
trick: the EMOD baseline itself gains a matching ${\approx}3$ pt from
$5$-seed ensembling ($0.624{\to}0.655$) yet stays $+0.039$ ($+6.0\%$)
below our ensemble, and single-model---before any ensembling---the recipe
already leads $+7.4\%$.
