...
\begin{figure*}[!t]
    ...
    \caption{\textbf{Overview of \ours.} The EEG signal is processed by a frozen pretrained CBraMod backbone with trainable adapter layers. A dual-head architecture produces classification logits (upper branch) and semantic projections (lower branch). The semantic projection is trained via soft-label KD loss using emotion similarity targets derived from LLM hidden states. The LLM branch (bottom) runs offline to extract emotion vectors.}
    \label{fig:pipeline}
\end{figure*}
