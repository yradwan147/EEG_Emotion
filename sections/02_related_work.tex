...
shared assumption across the distillation, similarity, and contrastive
families is that the auxiliary signal provides information the task does
not (parameter-efficient adaptation is included as a capacity-side
comparison); this paper studies a regime where that added information
does not help.
...
motivation, developed primarily on resting-state, trait, and static or
briefly-presented stimuli. Our cohort signal on
...
plus a seed-specific within-class residual---is the between- vs
within-class geometry of neural collapse~\citep{papyan2020prevalence},
...
Our SOTA prescription operationalises this geometry as a deep
ensemble~\citep{fort2019deepensembles}: seed-ensembling
averages out the seed-specific residual variance, while the residual is
load-bearing (inference-time ablation); auxiliary supervision on the
saturated basin yields no gain.

\paragraph{Baselines.}
The FACED-9 baselines we compare against are
CBraMod~\citep{wang2025cbramod} ($0.551$ published; $0.572$ our
reproduction) and
