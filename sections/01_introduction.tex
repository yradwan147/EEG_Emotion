...
\emph{Top:} a valence axis (\emph{V-axis}) from nine language-model emotion
stories orders the nine FACED emotions; the \emph{same} ordering appears in the
cohort human EEG ($r{=}0.87$ over $28$ clips) and, without ever being supervised
on it, in a converged EEG classifier's class means ($|r|\in[0.60, 0.77]$
across ten checkpoints). Thin lines mark the language model's positions.
...
ensemble the language-derived prototypes are indistinguishable from
random orthonormal vectors ($\Delta{=}{-}0.004$ BACC, $p{=}0.37$).
...
subjects, and the within-subject effect is near zero ($r{=}0.03$).
