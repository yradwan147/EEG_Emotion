...
\cref{fig:cluster_tsne_3panel} provides extended visualizations comparing baseline, KD, and adapter64 feature spaces. The progressive improvement in cluster separation is visually striking: baseline features show substantial class overlap (particularly among negative emotions), while KD and adapter64 create more discrete, well-separated clusters. Convex hull analysis (\cref{fig:cluster_convex_hull}) quantifies the reduced overlap, and the centroid distance heatmap (\cref{fig:cluster_centroids_heatmap}) confirms that KD increases inter-class separation uniformly across emotion pairs.
